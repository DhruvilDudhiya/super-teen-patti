...
class CardPrefab extends Phaser.GameObjects.Container {
	...
	constructor(scene, x, y) {
		super(scene, x ?? -10, y ?? -15);

		// frontCard
		const frontCard = scene.add.image(13, 17, "frontCard");
		frontCard.scaleX = 0.3;
		frontCard.scaleY = 0.3;
		this.add(frontCard);

		// cardIcon
		const cardIcon = scene.add.image(11, 16, "Diamond");
		cardIcon.scaleX = 0.2;
		cardIcon.scaleY = 0.2;
		this.add(cardIcon);

		// cardNumber
		const cardNumber = scene.add.text(-29, -51, "", {});
		...
		// small_cardIcon
		const small_cardIcon = scene.add.image(-17, -10, "Diamond");
